...
import { AUTHORIZE_URL, GUILD_REDIRECT_URI, PSD_CLIENT_ID } from "../../util/constants";
...
export default () => {
  ...
  const handleInviteClick = useCallback((event: MouseEvent<HTMLButtonElement>) => {
    ...
    guildInviteUrl.searchParams.set('guild_id', event.currentTarget.dataset.guild_id as string);

    if (popupWindow === null || popupWindow.closed) {
      const newPopupWindow = window.open(guildInviteUrl, "Invite Psifi", 'width=500,height=850');
  ...
  return (<BasePage title="Select a server">
    <div className="flex flex-row flex-wrap justify-center mx-auto">
      {
        guilds && guilds.filter(guild => guild.access).map(guild => <SelectGuildCard key={guild.id} guild={guild} />)
      }
      {
        guilds && guilds.filter(guild => !guild.access).map(guild => <InviteGuildCard key={guild.id} guild={guild} inviteHandler={handleInviteClick} />)
      }
    </div>
  </BasePage>)
}